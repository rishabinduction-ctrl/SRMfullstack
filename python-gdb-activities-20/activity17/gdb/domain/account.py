...
class Account(IAccount):
    ...
    def deposit_with_transaction(self, amount: float) -> Transaction:
        self.deposit(amount)
        return self.build_transaction(
            TransactionType.DEPOSIT, amount, self._account_number, self._account_number, f"Deposit of Rs. {amount:,.2f}"
        )

    def withdraw_with_transaction(self, amount: float, pin: int) -> Transaction:
        self.withdraw(amount, pin)
        return self.build_transaction(
            TransactionType.WITHDRAW, amount, self._account_number, self._account_number, f"Withdrawal of Rs. {amount:,.2f}"
        )
