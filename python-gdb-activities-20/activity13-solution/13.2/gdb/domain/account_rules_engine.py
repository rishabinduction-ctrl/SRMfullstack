...
class AccountRulesEngine:
    ...
    def get_minimum_balance(cls, account_type: str) -> float:
        if not account_type: return 0.0
        return cls._RULES.get(account_type.strip().upper(), {}).get("min_balance", 0.0)

    @classmethod
    def get_interest_rate(cls, account_type: str) -> float:
        if not account_type: return 0.0
        return cls._RULES.get(account_type.strip().upper(), {}).get("interest_rate", 0.0)

    @classmethod
    def get_overdraft_limit(cls, account_type: str) -> float:
        if not account_type: return 0.0
        return cls._RULES.get(account_type.strip().upper(), {}).get("overdraft_limit", 0.0)
